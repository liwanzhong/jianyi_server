...
<script type="text/javascript">
	...
	$(function() {
		$('#roleGrantForm').form({
			url : '${ctx}/instrument/checkItemReleationConfig/config.shtml',
			onSubmit : function() {
				progressLoad();
...
<div id="roleGrantLayout" class="easyui-layout" data-options="fit:true,border:false" style="width: 100%">
	主关联项：<span style="color: red">《${checkSmallItemFormMap.name}》</span>关联配置项
	<form id="roleGrantForm" method="post" action="${ctx}/instrument/checkItemReleationConfig/config.shtml">
		<input name="dist_check_id" type="hidden"  value="${checkSmallItemFormMap.id}" readonly="readonly">
								...
								<c:if test="${smallItem.big_item_id == bigItem.id}">
									<tr>
										<td>
											...
											<c:forEach items="${checkItemReleationConfigMapList}" var="checkItem">
												<c:if test="${checkItem.org_check_id == smallItem.id}">
