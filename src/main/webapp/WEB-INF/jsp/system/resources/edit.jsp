...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#pid').combotree({
			url : '${ctx}/resource/tree.shtml',
			parentField : 'pid',
			lines : true,
			panelHeight : 'auto',
			value : '${resFormMap.pid}'
		});

		$('#resourceEditForm').form({
			url : '${pageContext.request.contextPath}/resource/update.shtml',
			onSubmit : function() {
				progressLoad();
		...
		$("#resFormMap_ishide").val('${resFormMap.ishide}');
		$("#resFormMap_type").val('${resFormMap.type}');
	});
</script>
	...
	<form id="resourceEditForm" method="post">
		<table  class="grid">
			<tr>
				<td>资源名称</td>
				<td>
					<input name="resFormMap.id" type="hidden"  value="${resFormMap.id}">
